...
for (const moduleRecord of modules) {
  const files = typeof moduleRecord.path === 'string' ? moduleRecord.path.split(';').map(value => value.trim()) : [];
  ...
  for (const claimed of moduleRecord.exports || []) if (!runtimeExports.has(claimed)) add('MODULE_DECLARED_EXPORT_NOT_FOUND',`${moduleRecord.id}: ${claimed}`);
}
...
for (const moduleRecord of modules) for (const target of moduleRecord.calls || []) if (sets.module.has(target) && !callsEdges.has(`${moduleRecord.id}\0${target}`)) add('MODULE_DEPENDENCY_PROJECTION_MISMATCH',`${moduleRecord.id} calls ${target} without CALLS edge`);
for (const edge of dependencyEdges) if (edge.relation === 'CALLS' && sets.module.has(edge.source) && sets.module.has(edge.target) && !modules.find(moduleRecord => moduleRecord.id === edge.source)?.calls?.includes(edge.target)) add('MODULE_DEPENDENCY_PROJECTION_MISMATCH',`${edge.source} -> ${edge.target} missing in modules.calls`);
