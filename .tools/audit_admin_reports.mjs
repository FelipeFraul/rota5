...
async function seedData() {
  ...
  await dbInsert("admin_users", { phone: ROOT_PHONE, role: "root", status: "active", name: `${PREFIX} Diretor`, passphrase_hash: passphraseHash });
  await dbInsert("admin_users", { phone: MANAGER_PHONE, role: "admin", status: "active", name: `${PREFIX} Gerente`, passphrase_hash: passphraseHash });
  await dbInsert("admin_users", { phone: OPERATOR_PHONE, role: "operator", status: "active", name: `${PREFIX} Operador`, passphrase_hash: passphraseHash });
  ...
  ids = { ...ids, venueId, sectionId, eventId, sessionId };
...
async function sendMessage(phone, text) {
  ...
  while (Date.now() - startedAt < 10_000) {
    if (zapiMessages.length > before) {
      const message = zapiMessages.at(-1);
...
async function login(phone, label) {
  let response = await sendMessage(phone, "admin");
  assertIncludes(response.text.toLowerCase(), "palavra", `${label} pede senha`);
  response = await sendMessage(phone, PASS);
  assertIncludes(response.text, "MENU ADMIN", `${label} abre menu`);
  return response;
}

async function snapshotOperationalCounts() {
  const tables = ["orders", "payments", "reservations", "tickets", "session_seats", "courtesies", "ticket_validation_events"];
  const result = {};
...
async function runReport(option, label, expected = []) {
...
async function main() {
  ...
  try {
    ...
    await runReport(1, "E) Resumo geral", ["RESUMO GERAL", "Vendas pagas", "Pedidos pagos", "Cortesias emitidas"]);
    await runReport(2, "F) Vendas por evento", ["VENDAS POR EVENTO", "Valor vendido bruto", "Cortesias emitidas"]);
    ...
    response = await sendMessage(ROOT_PHONE, "10");
