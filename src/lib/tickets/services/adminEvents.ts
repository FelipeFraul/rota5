...
export function parseBrazilianDateTime(value: string) {
  const match = value
    .trim()
    .match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})(?:\s+|\s+Ã s\s+)(\d{1,2}):(\d{2})$/i);
...
export function parseSeatRange(value: string) {
  const match = value
    .trim()
    .match(
      /^(?:prefixo\s+)?(?:fileira\s+)?([a-zA-Z]+)\s*,?\s*(?:(\d+)\s*assentos?)?\s*,?\s*(?:de\s+)?(\d+)\s*(?:a|atÃ©|-)\s*(\d+)$/i,
    );
...
function parseSeatRangeLayoutLine(value: string, rowIndex: number): ParsedSeatLayoutLine | null {
  const match = value
    .trim()
    .match(
      /^(?:fileira\s+)?([a-zA-Z]+)\s+(?:(\d+)\s+assentos?\s+)?(?:de\s+)?(\d+)\s*(?:a|atÃ©|-)\s*(\d+)(.*)$/i,
    );
...
export async function duplicateAdminEvent(input: {
  ...
  const eventPayload = {
    title: `${sourceEvent.title} - CÃ“PIA`,
