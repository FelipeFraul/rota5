...
function renderAccessError() {
  return new NextResponse(
    `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Link expirado</title><style>:root{color-scheme:light;--app-bg-image:url("/rockbar.webp");font-family:Roboto,Arial,Helvetica,sans-serif}*{box-sizing:border-box}body{margin:0;background:#0b0f18;font-family:Roboto,Arial,Helvetica,sans-serif}.admin-login-shell{display:grid;min-height:100svh;place-items:center;padding:24px;background:var(--app-bg-image) center/cover no-repeat;color:#111827}.page-card-stack{display:grid;justify-items:center;width:min(100%,390px)}.brand-logo{display:block;width:min(340px,84vw);height:auto;margin:0 auto 18px;object-fit:contain;filter:drop-shadow(0 8px 18px rgba(0,0,0,.3))}.admin-login-card{width:100%;padding:28px 24px;border:1px solid #e5e7eb;border-radius:8px;background:#fff;box-shadow:0 22px 70px rgba(0,0,0,.18)}.admin-login-kicker{margin:0 0 10px;color:#006c67;font-size:13px;font-weight:900;letter-spacing:.12em;text-transform:uppercase}.admin-login-card h1{margin:8px 0 16px;color:#020617;font-size:28px;font-weight:900;line-height:1.08}.admin-login-card p{margin:10px 0;color:#334155;font-size:16px;font-weight:400;line-height:1.45}</style></head><body><main class="admin-login-shell"><div class="page-card-stack"><img class="brand-logo" src="/logo_rockbar.webp" alt="RockBar Pub"><section class="admin-login-card"><p class="admin-login-kicker">Acesso nao liberado</p><h1>Link expirado</h1><p>Volte ao WhatsApp, envie a opcao 2 novamente e abra o novo link de edicao.</p></section></div></main></body></html>`,
    {
      status: 401,
      headers: {
