...
async function buildEventDashboard(eventId: string) {
  const supabase = getSupabaseAdmin();
  const [ticketsResult, seatsResult, pricesResult, combosResult, redemptionsResult, validationsResult] = await Promise.all([
    supabase
      .from("tickets")
      .select(
        "id, status, issued_at, used_at, event_sessions!inner(event_id), venue_sections(id, name), reservation_items(ticket_type, price_cents, fee_cents), orders!inner(status, created_at)",
      )
    ...
    supabase
      ...
      .select("id, status, quantity, used_at")
      .eq("event_id", eventId)
      .returns<DashboardComboRedemptionRow[]>(),
    supabase
      .from("ticket_validation_events")
      .select("result, tickets!inner(id, event_sessions!inner(event_id))")
  ...
  for (const ticket of ticketsResult.data ?? []) {
    ...
    if (ticket.status === "used" || ticket.used_at) usedTickets += 1;
  ...
  for (const combo of combosResult.data ?? []) {
    if (combo.status === "pending_payment") comboOrdersPending += 1;
  ...
  const comboUsed = redemptions
    .filter((redemption) => redemption.status === "used" || redemption.used_at)
    .reduce((total, redemption) => total + (Number(redemption.quantity) || 0), 0);
  ...
  return {
    summary: {
      ...
      checkins: (validationsResult.data ?? []).length,
      usedTickets,
      comboOrdersPaid,
      comboOrdersPending,
      comboItemsSold,
...
export async function GET(request: Request, context: RouteContext) {
  ...
  if (url.searchParams.get("dashboard") === "1") {
    ...
    try {
      const dashboard = await buildEventDashboard(eventId);
