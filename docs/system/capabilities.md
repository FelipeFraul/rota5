...
## Baseline 2.7.0 capability reconciliation

`combo.kitchen_release`, `combo.prepare`, `combo.delivery_prompt`, `combo.delivery_choose` and `combo.redeem` now reference the serialized database transitions. Their primary user contracts remain compatible; the internal concurrency contract changed.
