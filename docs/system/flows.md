...
Dos 44 objetos de tabela, 42 participam diretamente de flows. `courtesy_limits` pertence à capability standalone órfã `courtesy.event_limit`; `seat_map_renders` permanece um artefato sem consumidor funcional confirmado. Das 33 funções SQL, 26 aparecem diretamente; as sete restantes são componentes técnicos já justificados no Capability Catalog. Dos 33 triggers, um (`reservations_sync_official_table_map_status`) produz transição funcional explícita; os outros 32 mantêm `updated_at` por `set_updated_at`.
