...
## Baseline 2.9.0 data model

The 00600 adds two SQL function identities, redefines ten existing functions and adds `whatsapp_outbound_deliveries_combo_operational_due_idx`; it adds no table or sequence. Operational combo decisions write durable outbound intents atomically with serialized redemption state. The migration ledger is aligned through 00600.


## Baseline 2.7.0 combo redemption state

`combo_redemptions.raw_metadata` remains JSONB, but critical CURRENT transitions now lock the row and merge against database-current state. `validate_combo_redemption` atomically persists consumption and delivery metadata. Table count remains 45; no historical row was backfilled by this freeze.
...
Referência canônica corrente: commit `0a10618648fc3f873afffd8f60e60bd0396b62e7`. O schema local resultante foi reconstruído sobre 87 migrations; o catálogo atual contém 45 tabelas, 57 funções SQL e 1 sequence. A sequence gera apenas novos códigos e não reescreve códigos históricos.
