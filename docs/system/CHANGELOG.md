...
**MINOR_COMPATIBLE_FUNCTIONAL_CHANGE.** Canonical functional source `c7ed2c31eb9c322ef489e71bb59631c39928a1e0` (commits `bb582a6` and `c7ed2c31`) adds durable combo operational delivery and legacy recovery. Migration `20260915000600` was applied by official Supabase CLI 2.117.0 db push after the separate 140001–150005 ledger reconciliation. Quality Gate 35141969033: Node 291/291, PostgreSQL 4/4, typecheck/lint/build PASS. Production `dpl_DmWaACtXLsGd7mPwsKKsk8s2n4gc` is READY on exact source, aliases and health PASS. The direct notification concurrency finding is RESOLVED; ambiguous external ACK remains ACTIVE separately. Documentary baseline commit: SELF_NOT_RECORDED.


## Baseline 2.8.1 — PATCH_DOCUMENTARY_CORRECTION

Corrected runtime export inventories, the proxy-to-rate-limit CALLS edge and stale CURRENT source-scope metadata. The dependency graph now contains 818 edges (616 direct, 138 indirect, 64 external). Functional source, finding lifecycle, database and Production deployment remain unchanged.

## Baseline 2.8.0 — MINOR_COMPATIBLE_FUNCTIONAL_CHANGE

- Hardened rate-limit outage handling with explicit `allowed | rate_limited | unavailable`.
- 14 public boundaries fail closed with 503; 5 strong-auth boundaries degrade explicitly after authentication.
- Cancelable 2s timeout; malformed responses no longer become false 429.
- Source `c48405e41df3d1cd69eb3d383b7c6dd17257155e` passed Quality Gate 35049217326: 285/285 Node, 4/4 PostgreSQL, typecheck, lint and build.
- Production `dpl_2bitbdQynYB6QdMiEkB1em65HsAy` serves the exact source by authoritative Vercel metadata; health is HTTP 200.
- `risk.rate-limit-fails-open` is RESOLVED; no database migration.

## Baseline 2.7.4 — resolved-finding and health projection reconciliation

- Reconciled `baseline-v1.resolved_findings` and every structurally derivable per-domain health projection with the canonical finding lifecycle.
- No finding lifecycle, functional source, database, deployment or runtime behavior changed.
...
- Type: `PATCH_DOCUMENTARY_CORRECTION`; canonical functional source `1f504eb4e4a08ab8f8de3ff3a39e1803f625dc27`; fingerprint `9a9ab1a24c824a879213174a34ba1940eded2eeaf90fca26494a6eb24bc9dbee`.
- Restored historical baseline attribution accidentally relabeled during 2.7.1 and reconciled current freeze/generation timestamps.
- No functional source, database, deployment, health or finding lifecycle changed. Baseline commit: `SELF_NOT_RECORDED`.

## Baseline 2.7.1 — factual CURRENT projection correction

- Type: `PATCH_DOCUMENTARY_CORRECTION`; canonical functional source `1f504eb4e4a08ab8f8de3ff3a39e1803f625dc27`; fingerprint `9a9ab1a24c824a879213174a34ba1940eded2eeaf90fca26494a6eb24bc9dbee`.
- Corrected stale CURRENT finding invariants/evidence, the combo resolution projection and one semantically false finding relation.
- No functional source, database, deployment, health or lifecycle changed. Baseline commit: `SELF_NOT_RECORDED`.
...
P1 now explicitly has no operational findings; the resolved historical P1 is under RESOLVED, both operational P2 findings are under P2, and current Node evidence is 276/276. No functional source, database, deployment, lifecycle, count, release-gate or health change occurred.

## Baseline 2.6.1 — temporal metadata correction

This PATCH_DOCUMENTARY_CORRECTION replaces impossible future CURRENT generation timestamps with the real execution clock and updates the findings methodology from stale Baseline 2.5.x wording to Baseline 2.6.1. Functional source, database, deployment, finding lifecycle, release gate and health are unchanged. The previous 2.6.0 freeze time is evidenced by Git commit `e5870d9864914b990274e3a99342d53c2810c710` at `2026-09-15T12:45:09-03:00`.

## Baseline 2.6.0 — durable paid delivery freeze

- Functional source: `0a10618648fc3f873afffd8f60e60bd0396b62e7`; SemVer: `MINOR_COMPATIBLE_FUNCTIONAL_CHANGE`.
- Paid TICKET and COMBO confirmation now atomically persists recoverable delivery work.
- `risk.payment-confirmed-before-external-delivery` is RESOLVED; `risk.paid-delivery-ambiguous-external-ack` is the separate P2 ACTIVE residual.
- Local delivery semantics are at-least-once; exactly-once external is not proven.
- Quality Gate 34984961888: 276/276 Node, 4/4 PostgreSQL, typecheck/lint/build PASS.

## Baseline 2.5.3 — non-self-referential runtime documentation

- **PATCH_DOCUMENTARY_CORRECTION** on unchanged canonical functional source `e931d66d03a620d5e26588c8f6c8714c62ef5d1d` and fingerprint `6483294a8c2a4e758fdb965f2f9dc41bef5c539b064b9d727239a3ccd6059954`.
- Reclassified `runtime.git-production` so the canonical functional source remains stable while the live branch tip is volatile and externally verified.
- Generated and validator-locked all CURRENT runtime Markdown rows against `runtime-validation.json`; the old Etapa 5 table is explicitly historical.
- Preserved the three-day functional stabilization cycle as `DOCUMENTED_AND_CLOSED`.

## Baseline 2.5.2 — final CURRENT projection reconciliation

- **PATCH_DOCUMENTARY_CORRECTION** on unchanged source `e931d66d03a620d5e26588c8f6c8714c62ef5d1d` and fingerprint `6483294a8c2a4e758fdb965f2f9dc41bef5c539b064b9d727239a3ccd6059954`.
- Derived DATA, RISKS and RUNTIME scorecard values from canonical machine-readable catalogs and added validator enforcement.
- Revalidated Supabase PostgREST OpenAPI with a read-only GET: 45 tables and 42 RPC routes measured directly; no mutation performed.
- Preserved the three-day functional stabilization history and all explicitly historical snapshots.
...
## Baseline 2.4.4 — universal RESOLVED semantics

- Type: `PATCH_DOCUMENTARY_CORRECTION`; functional source fingerprint unchanged.
- Derived and normalized all 11 `RESOLVED` findings without changing lifecycle, severity or priority.
- Top-level fields now represent current state; original title, description, evidence, impact, workaround, direction and priority justification are preserved in `resolution.historical_evidence`.
- Every current top-level evidence entry is marked `CURRENT`; complete `current_state` is validator-required.
- Resolved semantic invariants, aggregation invariants and semantic audit pass; release blockers remain 0.
