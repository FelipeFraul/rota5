...
Reusable fixed gate credentials are stored separately in:

```text
supabase/migrations/20260717000200_create_fixed_gate_accesses.sql
```

`public.fixed_gate_accesses` stores one active credential per normalized phone, the PBKDF2 passphrase hash, the owning admin, and revocation audit fields. It does not bind the credential permanently to an event. After authenticating, the validator can choose only published events whose `created_by_admin_user_id` matches `owner_admin_user_id`. The table has RLS enabled, denies `public`, `anon`, and `authenticated`, and grants access only to `service_role`.
...
The gate submenu options `5. Adicionar portaria fixa` and `6. Excluir portaria fixa` manage reusable credentials. A fixed validator sends `Portaria`, enters the fixed passphrase, chooses one of the published events owned by the admin who registered the phone, and receives a temporary event-scoped scanner link. Deletion is a confirmed soft revocation so audit history is retained.
...
- create event with title, artist, city, UF, venue, event date/time, and final status choice;
...
Current event menu behavior:

- `MEUS EVENTOS`: shows `1. Listar meus eventos`, `2. Criar meu evento`, `3. Editar meu evento`, `4. Ativar/Pausar meu evento`, `5. Duplicar evento`, `6. Voltar`, and `7. Sair`. The former `Setores e assentos` shortcut is no longer a main menu item; sections and seats are handled inside creation/editing flows.
- `LISTAR MEUS EVENTOS`: filters active (`published`), paused (`draft`), cancelled (`cancelled`), and all statuses. Lists are paginated at 5 events per page, `Mais` fetches the next page, numeric selection opens details, and `Voltar`/`Cancelar` return to `MEUS EVENTOS`.
...
- `2. Criar evento`: collects title, artist, city, UF, venue, cover photo, number of dates, sessions per date, each date, each time for that date, entry/place model, ticket-load model, ticket types/offers, numbered seats when applicable, and general information (`events.description`). It then shows the full summary, asks for `CONFIRMAR` or `CANCELAR`, asks for the final status choice (`draft` or `published`), and only then writes to the database. The date/session flow creates `dates_count * sessions_per_date` `event_sessions`, so 3 dates with 1 session create 3 sessions, and 1 date with 2 sessions creates 2 sessions on the same date with distinct times. The photo step accepts a WhatsApp image or a public `https://...` URL. Draft events may skip the photo, but `published` events cannot be created without one. The entry model step supports: one unnumbered entry, multiple unnumbered entries/sections, or numbered-seat sections. For shared capacity, the admin informs a total load and then each offer as `nome | valor | taxa opcional`; for separate capacity, each section/type is sent as `nome | capacidade | valor | taxa opcional`. Numbered-seat sections collect the typed seat map during creation and create structural seats plus `session_seats` immediately. Unnumbered entries create internal inventory units and `session_seats` as `available` so capacity can be reserved. It reuses an existing venue by name/city/UF or creates an active venue. The service always inserts the event as `draft` and sessions as `scheduled`; after sections, seats/units, and prices are created, it promotes the event to `published` and sessions to `sales_open` only when publication was the final choice. Cancelling before final status selection discards the conversation draft and leaves no temporary event. If date/session, initial-section, price creation, or final promotion fails after confirmation, the event remains as `draft`; no physical rollback delete is performed.
- `3. Editar evento`: shows `EDITAR MEU EVENTO` with `Editar nome`, `Editar artista`, `Editar cidade`, `Editar estado`, `Editar local`, `Editar foto`, `Editar data/hora`, `Editar setores/lugares`, `Editar carga`, `Editar valores`, `Editar informações gerais`, `Voltar`, and `Sair`. Simple fields require a confirmation summary before writing. Venue creation/reuse happens only after confirmation. Publishing is blocked until a cover photo exists. Setting status to `cancelled` requires `CANCELAR EVENTO`.
...
- Advanced session/date flow: lists sessions with date/time, status, local, section count, and price count. Creating a session accepts only `scheduled` or `sales_open` and does not create `session_seats`. Opening/closing sales uses `scheduled`, `sales_open`, or `sales_closed`. Cancelling a session requires `CANCELAR SESSÃO`. Changing session date/time is blocked for past dates and when reservations, orders, or tickets already exist.
- Advanced section/seat flow, inside event editing and not as a main `MEUS EVENTOS` item: lists sections with slug, capacity, numbered-seat flag, status, structural seat count, and session-seat count. It creates sections with normalized unique slugs, edits safe fields with confirmation, creates manual/range structural seats only for numbered sections, blocks/inactivates seats only when they are not reserved or sold in any session, and creates missing `session_seats` as `available` without touching existing `reserved`, `sold`, or `blocked` rows. `Editar carga` is supported for unnumbered sections: increasing creates available inventory units and reducing only blocks available units; it refuses reductions below already sold/reserved totals.
- Advanced price/lote flow: lists prices for all sessions with session, section, ticket type, label, price, fee, sale window, and status. It creates active `BRL` prices with optional sale start/end dates, converts decimal strings such as `10,90` and `10.90` to cents, maps `promo/courtesy` aliases to the schema values, allows multiple offers of the same `ticket_type` in a session/section, blocks duplicate labels for the same session/section, edits simple value by changing only `price_cents` after confirmation, and never deletes a price. Existing `reservation_items` keep their frozen price and fee.
- `5. Duplicar evento`: asks for confirmation, then creates a new `draft` event named with `- CÓPIA`, a new venue copy, new sessions, new sections, new seats/inventory units, new `session_seats`, and new ticket prices. It does not duplicate reservations, orders, payments, tickets, validation events, gate access/session rows, courtesies, or other financial/history data. After duplication, the admin is placed on the edit menu for the copied event.
...
- C: complete event creation creates venue, event, and date/session after confirmation;
