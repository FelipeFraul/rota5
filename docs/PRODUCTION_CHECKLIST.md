...
- Fase 2 de hardening de tokens deve permanecer ativa: `/admin/login/{token}` e `/gate/session/{token}` usam o token somente no bootstrap server-side, criam cookie `HttpOnly` e redirecionam para `/admin/login` e `/gate/session`. Depois disso, `/api/admin/login/verify`, `/api/gate/session/validate` e `/api/gate/session/scan` usam cookie, sem token bruto em props ou body.
- Cookies esperados: `admin_login_challenge` e `gate_session`, ambos `HttpOnly`, `Secure`, `SameSite=Lax`, `Path=/` e `Max-Age` curto/alinhado ao challenge ou sessão de portaria. O `Path=/` é intencional porque as páginas limpas e APIs ficam em árvores diferentes.
- Visibilidade esperada por design: token de admin/portaria pode aparecer apenas no primeiro carregamento do link de bootstrap, senha digitada no POST do próprio navegador, código único exibido ao admin, token do QR lido no request de validação e dados operacionais mínimos.
