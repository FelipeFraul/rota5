...
- Visibilidade esperada por design: token de admin na URL, token de portaria na URL, senha digitada no POST do próprio navegador, código único exibido ao admin, token do QR lido no request de validação e dados operacionais mínimos. Fase 2 futura: cookie `HttpOnly` e limpeza de URL para tokens de acesso.
