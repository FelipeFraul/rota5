...
- `img-src data:` é obrigatório para QRCode/mapas base64. Em produção, `script-src` e `style-src` não devem usar `'unsafe-inline'` nem `'unsafe-eval'`; o `proxy` gera nonce por request para scripts/estilos do Next e mantém `https://sdk.mercadopago.com` para o checkout Mercado Pago. Em desenvolvimento local, `'unsafe-eval'` e `'unsafe-inline'` podem aparecer apenas para compatibilidade do React/Next dev server.
- Páginas e APIs não devem emitir `Access-Control-Allow-Origin`: os fluxos do navegador são same-origin e os webhooks são server-to-server. Assets `_next/static` sobrescrevem o wildcard automático da Vercel com a origem canônica.
- `robots.txt` deve existir para evitar indexação acidental de rotas tokenizadas e APIs. Ele não é mecanismo de segurança; admin, portaria, tickets e API continuam protegidos pelo backend.
- `sitemap.xml` pode permanecer 404 enquanto não houver páginas públicas reais para indexar. Não incluir links de tickets, portaria, admin, tokens ou URLs temporárias em sitemap.
- `/.well-known/security.txt` só deve ser publicado quando houver um contato operacional seguro para divulgar. Não publicar e-mail pessoal ou segredo.
...
- Fase 1 de minimização de dados no front deve permanecer ativa: admin login não retorna timestamps ISO exatos de link/código; portaria não retorna `status`, `expiresAt` exato ou final do telefone do validador no DTO público; scan público não retorna `ticketCode` nem objeto `ticket`, apenas `allowed`, `result`, `message` e, quando permitido, `section`/`seat`.
- Visibilidade esperada por design: token de admin na URL, token opaco de portaria na URL, senha digitada no POST do próprio navegador, código único exibido ao admin, token do QR lido no request de validação e dados operacionais mínimos. Token de portaria deve ser aleatório, sem payload decodificável com `gid`, telefone ou expiração; `gate_sessions` deve salvar apenas `token_hash`. Links antigos de portaria no formato payload assinado devem ser descartados e regenerados após deploy. Fase 2 futura: cookie `HttpOnly` e limpeza de URL para tokens de acesso.
