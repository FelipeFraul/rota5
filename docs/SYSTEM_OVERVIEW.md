...
> 4. Revogar acessos
> 5. Adicionar portaria fixa
> 6. Excluir portaria fixa
> 7. Voltar
> 8. Sair
```
...
`Adicionar portaria fixa` pede uma única vez o telefone e uma palavra-chave fixa. A palavra-chave é exibida na confirmação, mas o banco armazena somente seu hash PBKDF2. Um telefone pode ter somente uma portaria fixa ativa no sistema.

Quando o telefone cadastrado envia `Portaria` para `+55 15 99834-3191`, o sistema pede a palavra-chave fixa. Depois de validá-la, mostra somente os eventos publicados cujo `created_by_admin_user_id` seja o Diretor ou Gerente que cadastrou aquela portaria. A escolha do evento gera uma nova `gate_session` temporária e restrita ao evento selecionado.

`Excluir portaria fixa` lista somente os cadastros pertencentes ao administrador autenticado, mascara os telefones e exige confirmação com `SIM`. A exclusão revoga a credencial sem apagar seu histórico de auditoria. Mensagens recebidas com a palavra-chave fixa são redigidas como `[GATE_ACCESS_REDACTED]`.
