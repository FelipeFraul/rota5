...
Na Fase 2, os links tokenizados de admin e portaria passaram a usar o token da URL somente como bootstrap inicial. `/admin/login/{token}` valida o challenge no servidor, grava o cookie `admin_login_challenge` com `HttpOnly`, `Secure`, `SameSite=Lax`, `Path=/` e `Max-Age` alinhado ao challenge, e redireciona para `/admin/login`. `/gate/session/{token}` faz o mesmo para o cookie `gate_session`, com validade alinhada a `gate_sessions.expires_at`, e redireciona para `/gate/session`. Depois do bootstrap, `/api/admin/login/verify`, `/api/gate/session/validate` e `/api/gate/session/scan` usam somente o cookie `HttpOnly`; o token bruto não volta em props nem em body de requests do browser.

Continuam visíveis por design: o token aparece na barra de endereço apenas durante o bootstrap inicial do link, a senha digitada segue no POST do próprio navegador, o código único continua sendo exibido ao admin para envio no WhatsApp, o token do QR lido pelo scanner segue no request de validação porque o navegador lê o QR, e dados operacionais mínimos como evento/sessão/setor/assento podem aparecer quando necessários para operação.
...
O sistema pede uma palavra-chave individual no cadastro e salva apenas o hash PBKDF2 em `admin_users.passphrase_hash`. A palavra-chave não aparece na confirmação, não é salva em texto puro no histórico de mensagens e o contexto temporário guarda apenas o hash pendente até a confirmação. Para entrar, o administrador envia `admin`, recebe um link temporário válido por 2 minutos, informa a senha individual na página `/admin/login/{token}` e envia no WhatsApp o código de uso único exibido, também válido por 2 minutos. A tabela `admin_login_challenges` guarda apenas hash SHA-256 do token do link, hash SHA-256 do código e hash da origem/IP quando disponível; o link enviado é salvo no histórico como `[ADMIN_LOGIN_LINK_REDACTED]` e o código recebido fica coberto pela redação de autenticação. Não existe senha geral nem fallback por variável de ambiente; administrador ativo sem `passphrase_hash` não autentica e a constraint `admin_users_active_requires_passphrase_hash` impede manter ou ativar admin sem hash individual. Admin desativado pode ficar sem hash, mas precisa receber uma nova palavra-chave para voltar a ficar ativo. Se o telefone já existir ativo, não cria duplicado. Se existir desativado, o fluxo pode reativar com confirmação `REATIVAR ADMIN` e define uma nova palavra-chave.
