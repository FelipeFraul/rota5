...
begin
  ...
  v_contacts_count := pg_catalog.jsonb_array_length(p_contacts);
  ...
  loop
    ...
    v_contact_name := pg_catalog.nullif(pg_catalog.btrim(v_contact ->> 'displayName'), '');
    v_contact_phone := pg_catalog.nullif(pg_catalog.btrim(v_contact ->> 'phone'), '');
    ...
    set
      ...
      participant_delivery_status = case
        when public.tickets.participant_delivery_status = 'delivered'
          then 'delivered'
        else 'awaiting_participant_request'
      end,
      participant_delivered_at = case
        when public.tickets.participant_delivery_status = 'delivered'
          then public.tickets.participant_delivered_at
        else null
      end
    where public.tickets.id = v_ticket.id;
...
revoke all on function public.assign_participant_contacts_to_order_tickets(uuid, jsonb) from public;
revoke all on function public.assign_participant_contacts_to_order_tickets(uuid, jsonb) from anon;
revoke all on function public.assign_participant_contacts_to_order_tickets(uuid, jsonb) from authenticated;
grant execute on function public.assign_participant_contacts_to_order_tickets(uuid, jsonb) to service_role;
