...
begin
    ...
    select
      ...
      count(t.id) filter (
        where t.status <> 'cancelled'
          and o.status = 'paid'
          and coalesce(ri.ticket_type, '') <> 'free'
          and coalesce(ri.price_cents, 0) + coalesce(ri.fee_cents, 0) > 0
      )::integer as items_sold,
      coalesce(sum(
        coalesce(ri.price_cents, 0) + coalesce(ri.fee_cents, 0)
      ) filter (
        where t.status <> 'cancelled'
          and o.status = 'paid'
          and coalesce(ri.ticket_type, '') <> 'free'
          and coalesce(ri.price_cents, 0) + coalesce(ri.fee_cents, 0) > 0
      ), 0)::integer as revenue_cents
  ...
  cards as (
    select
      ...
      coalesce(tsm.items_sold, 0) as ticket_items_sold,
      coalesce(tsm.revenue_cents, 0) as ticket_revenue_cents
    from summarized s
    left join ticket_metrics tm on tm.event_id = s.id
    left join ticket_sales_metrics tsm on tsm.event_id = s.id
  ),
  filtered_cards as (
    select *
    from cards c
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'ticketItemsSold', c.ticket_items_sold,
        'ticketRevenueCents', c.ticket_revenue_cents
      )
      order by
