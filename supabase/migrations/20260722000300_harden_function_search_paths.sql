-- Harden Supabase Function Search Path Mutable warnings for project functions.
-- Contracts, SECURITY INVOKER behavior, signatures, and returns are preserved.
...
CREATE OR REPLACE FUNCTION public.confirm_paid_ticket_order(p_order_id uuid, p_provider text, p_provider_payment_id text, p_amount_cents integer, p_paid_at timestamp with time zone DEFAULT now(), p_raw_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
SET search_path = ''
AS $function$
...
begin
  ...
  select
    ...
    encode(extensions.digest(gen_random_uuid()::text || clock_timestamp()::text || ri.id::text, 'sha256'), 'hex'),
    'issued',
    v_now
  from public.reservation_items ri
  where ri.reservation_id = v_reservation.id
...
CREATE OR REPLACE FUNCTION public.consume_rate_limit(p_route_key text, p_source_hash text, p_limit integer, p_window_seconds integer, p_now timestamp with time zone DEFAULT now())
 RETURNS jsonb
 LANGUAGE plpgsql
SET search_path = ''
AS $function$
...
declare
  ...
  v_beneficiary_phone text := regexp_replace(coalesce(p_beneficiary_phone, ''), '\D', '', 'g');
begin
...
begin
  ...
  select regexp_replace(coalesce(c.whatsapp_phone, ''), '\D', '', 'g')
  into v_phone
  from public.customers c
  where c.id = p_customer_id;

  if v_phone is null or btrim(v_phone) = '' then
    return jsonb_build_object('allowed', true, 'risk_event_id', null);
  end if;

  v_phone_hash := encode(extensions.digest(v_phone, 'sha256'), 'hex');
  v_source_hash := case
    when p_source_identifier is null or btrim(p_source_identifier) = '' then null
    else encode(extensions.digest(btrim(p_source_identifier), 'sha256'), 'hex')
  end;
  ...
  if v_created_count >= 5 then
    ...
    values (
      ...
      p_quantity,
      jsonb_build_object('limit', 5, 'window_minutes', 15),
  ...
  if v_churn_count >= 5 then
    ...
    values (
      ...
      p_quantity,
      jsonb_build_object('limit', 5, 'window_minutes', 30),
  ...
  if v_source_hash is not null then
    ...
    if v_source_count >= 20 then
      ...
      values (
        ...
        p_quantity,
        jsonb_build_object('limit', 20, 'window_minutes', 15),
        v_now + interval '15 minutes'
      );
  ...
  if v_recent_quantity + p_quantity > 10 then
    ...
    values (
      ...
      p_quantity,
      jsonb_build_object('limit', 10, 'window_minutes', 15),
      v_now + interval '15 minutes'
    );
...
CREATE OR REPLACE FUNCTION public.search_public_events_ranked(search_term text, date_from timestamp with time zone DEFAULT now(), date_to timestamp with time zone DEFAULT NULL::timestamp with time zone, result_limit integer DEFAULT 20)
